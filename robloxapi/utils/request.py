...
class Request:
    ...
    def __init__(self, cookie=None, debug=False, session=None):
        ...
        if session is None:
            raise Exception("No session was provided.")
    ...
    async def request(self, **kwargs):
        ...
        if r.status == 403 and r.headers.get('X-CSRF-TOKEN'):
            ...
            if not kwargs.get('noerror'):
                raise BadStatus(f'Got status {r.status} from {kwargs["url"]} data: {r.text}')
        if self.debug:
            print(kwargs['method'], kwargs['url'], r.status, r.text)
        return r
